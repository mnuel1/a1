...
export async function POST({ request }: RequestEvent) {
	try {
		...
		const existingShipment = await db
			.select()
			.from(shipments)
			.where(eq(shipments.shipmentNumber, manifestData.shipmentNo))
			.limit(1);
    ...
    // @ts-ignore
		const deliveryRows = manifestData.manifestData.map((row) => ({
		...
		await db.insert(deliveries).values(deliveryRows).onConflictDoNothing(); // Skip if trackingNumber or barcodeNo exists
