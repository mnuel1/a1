...
const CardManifest = ({ settings, deliveries, handleFieldChange, handleSubmit, status, isReadOnly = false }) => {
  ...
  return (
    <div className="space-y-8">
      {deliveries.map(item => (
        <div key={item.delivery_id} className="border rounded-lg p-6 shadow-sm bg-white max-w-5xl mx-auto text-sm">
          ...
          <div className="grid grid-cols-2 gap-4 mb-3">
            <div>
              ...
              {groupByRow[3]?.filter(f => f.cardKey.includes("shipper")).map(f => (
              ...
              {groupByRow[4]?.filter(f => f.cardKey.includes("shipper")).map(f => (
                <label key={f.cardKey} className="block mb-1">
                  <strong>{f.label}:</strong>{" "}
                  <input
                    className="border px-2 py-1 rounded w-full"
            ...
            <div>
              ...
              {groupByRow[3]?.filter(f => f.cardKey.includes("consignee")).map(f => (
                <label key={f.cardKey} className="block mb-1">
                  <strong>{f.label}:</strong>{" "}
                  <input
                    className="border px-2 py-1 rounded w-full"
              ...
              {groupByRow[4]?.filter(f => f.cardKey.includes("consignee")).map(f => (
                <label key={f.cardKey} className="block mb-1">
                  <strong>{f.label}:</strong>{" "}
                  <input
                    className="border px-2 py-1 rounded w-full"
          ...
          <div className="flex flex-col gap-3 mb-4">
            {/* Address row */}
            {groupByRow[5]?.filter(f => f.label.toLowerCase() === "consignee address").map(f => (
            ...
            <div className="flex gap-3">
              {groupByRow[5]
                ?.filter(f => f.label.toLowerCase() === "city" || f.label.toLowerCase() === "region")
                .map(f => (
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            {groupByRow[6]?.map(f => (
              <label key={f.cardKey} className="block">
                <strong>{f.label}:</strong>{" "}
                <input
                  type={f.type === "date" ? "date" : "text"} // render calendar for date fields
                  className="border px-2 py-1 rounded w-full"
                  value={item[f.cardKey] || ""}
                  onChange={(e) =>
                    handleFieldChange(item.delivery_id, f.cardKey, e.target.value)
    ...
    ${isReadOnly ? 'bg-gray-400 cursor-not-allowed' : 'bg-primary hover:bg-primary-60 cursor-pointer'}`}
          >
            Update
          </button>
